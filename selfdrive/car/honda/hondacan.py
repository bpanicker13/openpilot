...
def create_brake_command(apply_brake, pcm_override, pcm_cancel_cmd, chime, idx):
  ...
  bus = 0
  if car_fingerprint in (CAR.CLARITY):
    bus = 2
  commands.append(make_can_msg(0x1fa, msg, idx, bus))
  return commands
...
def create_steering_control(apply_steer, car_fingerprint, idx):
  ...
  commands = []
  if car_fingerprint in (CAR.CLARITY): #TODO- Fix this if this doesn't work
    bus = 2
  else bus = 0
  if car_fingerprint in (CAR.CRV, CAR.ACURA_RDX):
    msg_0x194 = struct.pack("!h", apply_steer << 4) + ("\x80" if apply_steer != 0 else "\x00")
    commands.append(make_can_msg(0x194, msg_0x194, idx, bus))
  else:
...
def create_ui_commands(pcm_speed, hud, car_fingerprint, idx):
  ...
  commands.append(make_can_msg(0x30c, msg_0x30c, idx, bus))

  msg_0x33d = chr(hud.X5) + chr(hud.lanes) + chr(hud.beep) + chr(hud.X8)
  commands.append(make_can_msg(0x33d, msg_0x33d, idx, bus))
  if car_fingerprint in (CAR.CIVIC, CAR.ODYSSEY):
    msg_0x35e = chr(0) * 7
    commands.append(make_can_msg(0x35e, msg_0x35e, idx, bus))
    msg_0x39f = (chr(0) * 2 + chr(hud.acc_alert) + chr(0) + chr(0xff) + chr(0x7f) + chr(0))
    commands.append(make_can_msg(0x39f, msg_0x39f, idx, bus))
  return commands
